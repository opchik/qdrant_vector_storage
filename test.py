...
async def async_main() -> None:
    ...
    file_upload_result = await client.upload_markdown(
        collection_name=COLLECTION_NAME,
        md_input=MD_FILE,
        processor=processor,
        batch_size=100,
    ...
    print(f"Uploaded points: {file_upload_result.chunks_uploaded}")   
...
def sync_main() -> None:
    ...
    # 3) Connect to Qdrant
    client = QdrantSyncClient(url=QDRANT_URL)

    # 4) Create collection if missing
    exists = client.client.collection_exists(COLLECTION_NAME)
    if not exists:
    ...
    # 6) Upload points
    point_ids = client.upload_points(
        collection_name=COLLECTION_NAME,
        points=points,
        batch_size=100,
        wait=True,
    ...
    # file_upload_result = client.upload_markdown(
    #     collection_name=COLLECTION_NAME,
    #     md_input=MD_FILE,
    #     processor=processor,
    #     batch_size=100,
    ...
    # 7) Collection info
    info = client.get_collection_info(collection_name=COLLECTION_NAME)
    print(f"collection_info.name: {info.get('config', {}).get('params', {}).get('vectors', {}).get('size', 'n/a')}")
    print(f"collection_info: {info}")
    ...
    # 9) Count points
    total_points = client.count_points(collection_name=COLLECTION_NAME)
    print(f"total_points: {total_points}")

    # 10) Healthcheck
    flag = client.healthcheck()
    print(f"healthcheck: {flag}")
